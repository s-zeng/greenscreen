class Dashing.HotProgressBars extends Dashing.Widget
    ...
    drawWidget: (progress_items) ->
        ...
        if progress_items.length == 0
            ...
            progress_items.forEach (item) =>
                ...
                if elem.length
                    @animateProgressBarContent(elem[0], item, 0)
                ++counter
    ...
    animateProgressBarContent: (element, item, baseDuration) ->
        setHotProgressBarValue(element, item.progress, item.warning, item.critical, item.localScope)
        updateHotProgressBarStatus(this)

        #comment the above code and uncomment the below code to turn on fancy animation
